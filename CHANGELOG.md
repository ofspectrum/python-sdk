...
- Streaming encode now sends and receives on one thread so live first-window output cannot race the upload.
- Added `audio.open_stream_pool()` for thread-safe persistent streaming encode connections.
- Stream pools use stable opaque connection slots, reuse connections across operations, and close with `pool.close()` or `client.close()`.
- Before an operation sends input, the pool probes and replaces stale connections. Operations are never replayed automatically after input starts.

## 1.2.1 - 2026-08-18

- Exposed OneFile encode persistence and preflight flags that the product API already accepts: `save_file`, `keep_original`, `check_watermark`, `response_format`, and `original_filename`.
- Exposed decode `save_file` instead of hard-coding it.
- Defaults stay compatible with previous SDK behavior (`save_file=True`, `check_watermark=True`).
- `smooth` is the same product option on file encode and streaming encode.
- Added `audio.stream_encode()` for low-latency file encode. Media decode and rebuild libraries install with the SDK.
- `stream_encode()` accepts the previous no-storage flags (`save_file`, `keep_original`, `check_watermark`, `response_format`) and only allows the non-persisting values.
